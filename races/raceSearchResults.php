...
if (!isset($_POST['attributes'])){
    ...
    header ("Location: raceSearchPage.php");
}
...
<div id="goback"> 
<svg onclick="window.location.href = 'raceSearchPage.php'" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="80%" x="0px" y="0px" viewBox="0 0 261.2 39.4" style="enable-background:new 0 0 221.2 39.4;" xml:space="preserve"> <style type="text/css"> .st0{fill:#FFFFFF;} .st1{font-family:'Formula1-Display-Bold';} .st2{font-size:29px;} .st3{font-size:37px;} #Layer_1{cursor: pointer;} </style> <text transform="matrix(1 0 0 1 19.0044 28.4741)" class="st0 st1 st2">SEARCH PAGE</text> <text transform="matrix(1 0 0 1 -2.098083e-04 32.3751)" class="st0 st1 st3">&lt;</text> </svg>
</div> 

<div align="center" class="table-container">

<p class = "dbInfo" style = "font-size: 50px;font-weight: 700;margin-top:2%;margin-bottom:0%">RACE SEARCH RESULTS</p>
...
<thead> 
<tr> <th> DRIVER NUMBER </th> <th> GP NAME </th> <th>POSITION</th> </tr> 
</thead>
<tbody>
<?php
include "../config.php";
...
if (validatesAsInt($valueSelection)){ // you have an integer in value
  //echo($valueSelection);
  if($attributeSelection!="gpName"){ // you have integer and not search for GP

    if($operationSelection == "smallerEqual"){

    $sql_statement = "SELECT * FROM racesIn WHERE $attributeSelection <= $valueSelection";
...
while($row = mysqli_fetch_assoc($result))
{
  ...
  $position = $row['position'];
	...
	echo "<tr>" . "<td>" . $driverNumber . "</td>" . "<td>" . $gpName . "</td>" . "<td>" . $position . "</td>" ."</tr>";
}
...
elseif($operationSelection == "equal"){
  ...
  else{ // you search for GP which cannot be an integer

    echo '<script>
    alert("You tried to search GP with numerical value! Only enter string values to search GP!");
    window.location.href="raceSearchPage.php";
    </script>';
  ...
  if($attributeSelection!="gpName"){
    echo '<script>
    alert("You tried to search Driver or Position with a string value! Only enter integer values to search Driver or Position!");
    window.location.href="raceSearchPage.php";
    </script>';
  }
  else{
    ...
    if($operationSelection != "equal"){
      echo '<script>
      alert("You tried to search a GP with greater or smaller operations! Only use equal to operation for GP search!");
      window.location.href="raceSearchPage.php";
      </script>';
