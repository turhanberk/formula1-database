...
if (!isset($_POST['attributes'])){
    ...
    header ("Location: carSearchPage.php");
}
...
<div id="goback"> 
<svg onclick="window.location.href = 'carSearchPage.php'" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="80%" x="0px" y="0px" viewBox="0 0 261.2 39.4" style="enable-background:new 0 0 221.2 39.4;" xml:space="preserve"> <style type="text/css"> .st0{fill:#FFFFFF;} .st1{font-family:'Formula1-Display-Bold';} .st2{font-size:29px;} .st3{font-size:37px;} #Layer_1{cursor: pointer;} </style> <text transform="matrix(1 0 0 1 19.0044 28.4741)" class="st0 st1 st2">SEARCH PAGE</text> <text transform="matrix(1 0 0 1 -2.098083e-04 32.3751)" class="st0 st1 st3">&lt;</text> </svg>
</div> 

<div align="center" class="table-container">

<p class = "dbInfo" style = "font-size: 50px;font-weight: 700;margin-top:2%;margin-bottom:0%">CAR SEARCH RESULTS</p>
...
<thead> 
<tr> <th> CAR ID </th> <th> CAR ENGINE </th> <th>CHASSIS LENGTH</th> <th> BODY COLOR</th> </tr> 
</thead>
<tbody>
<?php
include "../config.php";
...
if (validatesAsInt($valueSelection)){ // you have an integer in value
  ...
  if($attributeSelection!="color"){ // you have integer and not search for string
    if($operationSelection == "smallerEqual"){

    $sql_statement = "SELECT * FROM Car WHERE $attributeSelection <= $valueSelection";
    ...
    while($row = mysqli_fetch_assoc($result))
    {
      ...
      echo "<tr>" . "<td>" . $carID . "</td>" . "<td>" . $carengine . "</td>" . "<td>" . $carlength . "</td>" . "<td>" . $color . "</td>"."</tr>";
    }
...
elseif($operationSelection == "equal"){
  ...
  else { // you search for color which cannot be an integer
    echo '<script>
    alert("You tried to search car color with numerical value! Only enter from [Red, Orange, Yellow, Green, Blue, Purple, Pink, Brown, Gray, Black, White] to search cars by color!");
    window.location.href="carSearchPage.php";
    </script>';
  } 
  ...
  if($attributeSelection!="color"){
    echo '<script>
    alert("You tried to search Car ID or Chassis Length with a string value! Only enter integer values to search them!");
    window.location.href="carSearchPage.php";
    </script>';
  }
  else{
    ...
    if($operationSelection != "equal"){
      echo '<script>
      alert("You tried to search a color with greater or smaller operations! Only use equal to operation for color search!");
      window.location.href="carSearchPage.php";
      </script>';
      ...
      if (!in_array($valueSelection, $colors)) {
        echo '<script>
        alert("You tried to search car color with numerical value! Only enter from [Red, Orange, Yellow, Green, Blue, Purple, Pink, Brown, Gray, Black, White] to search cars by color!");
        window.location.href="carSearchPage.php";
        </script>';
