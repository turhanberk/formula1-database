...
if (!isset($_POST['attributes'])){
    ...
    header ("Location: gpSearchPage.php");
}
...
<div id="goback"> 
<svg onclick="window.location.href = 'gpSearchPage.php'" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="80%" x="0px" y="0px" viewBox="0 0 261.2 39.4" style="enable-background:new 0 0 221.2 39.4;" xml:space="preserve"> <style type="text/css"> .st0{fill:#FFFFFF;} .st1{font-family:'Formula1-Display-Bold';} .st2{font-size:29px;} .st3{font-size:37px;} #Layer_1{cursor: pointer;} </style> <text transform="matrix(1 0 0 1 19.0044 28.4741)" class="st0 st1 st2">SEARCH PAGE</text> <text transform="matrix(1 0 0 1 -2.098083e-04 32.3751)" class="st0 st1 st3">&lt;</text> </svg>
</div> 

<div align="center" class="table-container">

<p class = "dbInfo" style = "font-size: 50px;font-weight: 700;margin-top:2%;margin-bottom:0%">GP SEARCH RESULTS</p>
...
<thead> 
<tr> <th> GP NAME </th> <th> CIRCUIT NAME </th> <th> WINNER </th> <th> WEATHER </th> <th> FASTEST LAP </th> </tr> 
</thead>
<tbody>
<?php
include "../config.php";
...
if (validatesAsInt($valueSelection)){ // you have an integer in value
  //echo($valueSelection);
  if($attributeSelection!="gpName" && $attributeSelection!="circuitName" && $attributeSelection!="weather"){ // you have integer and not search for string

    if($operationSelection == "smallerEqual"){

    $sql_statement = "SELECT * FROM grandPrix WHERE $attributeSelection <= $valueSelection";
    ...
    while($row = mysqli_fetch_assoc($result))
    {
      ...
      echo "<tr>" . "<td>" . $gpName . "</td>" . "<td>" . $circuitName . "</td>" . "<td>" . $winner . "</td>" . "<td>" . $weather . "</td>" . "<td>" . $fastestLap . "</td>" ."</tr>";
    }
...
elseif($operationSelection == "equal"){
  ...
  else{ // you search for string which cannot be an integer

    echo '<script>
    alert("You tried to search GP, Circuit, Weather with numerical value! Only enter string values with correct form to search them!");
    window.location.href="gpSearchPage.php";
    </script>';
  ...
  if($attributeSelection!="gpName" && $attributeSelection!="circuitName" && $attributeSelection!="weather"){
    echo '<script>
    alert("You tried to search winner driver a string value! Only enter integer values to search Driver!");
    window.location.href="gpSearchPage.php";
    </script>';
  }
  else{ // you are searching for a string in correct place

    if($operationSelection != "equal"){
      echo '<script>
      alert("You tried to search a GP, Circuit or Weather with greater or smaller operations! Only use equal to operation for them!");
      window.location.href="gpSearchPage.php";
      </script>';

    }
    else{
      if($attributeSelection=="weather" && $valueSelection!="Sunny" && $valueSelection!="Stormy" && $valueSelection!="Snowy" && $valueSelection!="Rainy" && $valueSelection!="Foggy" && $valueSelection!="Cloudy"){

        echo '<script>
        alert("Weather can only be Sunny, Stormy, Snowy, Rainy, Foggy, Cloudy! You cannot search anything else!");
        window.location.href="gpSearchPage.php";
        </script>';
      }
